...
def save_config(profile_listbox, profile_entry):
    ...
    data = {
        ...
        "launcher_directory": launcher_entry.get(),
...
def load_config():
    ...
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, "r") as file:
            ...
            launcher_entry.insert(0, data.get("launcher_directory"))
...
def apply_theme():
    ...
    if dark_mode_var.get():
        ...
        launcher_entry.config(bg="#3c3c3c", fg="#ffffff")
        ...
        launcher_setup_frame.config(bg="#2e2e2e")
        ...
        launcher_entry_frame.config(bg="#2e2e2e")
        ...
        launcher_directory_label.config(bg="#2e2e2e", fg="#ffffff")
        ...
        for widget in launcher_entry_frame.winfo_children():
            if isinstance(widget, tk.Button):
                widget.config(bg="#555555", fg="#ffffff")
...
def browse_launcher():
    """Opens a directory dialog for the launcher folder."""
    path = filedialog.askdirectory()
    if path:
        launcher_entry.delete(0, tk.END)
        launcher_entry.insert(0, path)
        save_config(profile_listbox, profile_entry)


def launch_bloodborne_launcher():
    """Launches the Bloodborne Launcher for easy access."""
    launcher_directory = launcher_entry.get()
    if not launcher_directory:
        messagebox.showerror("Error", "Set the launcher directory first!")
        return

    subprocess.run([
        'powershell', '-Command',
        f'Start-Process "{launcher_directory+"/BB_Launcher.exe"}" -WorkingDirectory "{launcher_directory}" -Verb RunAs'
    ])
...
launcher_setup_frame = tk.Frame(root)
launcher_setup_frame.pack()

launcher_directory_label = tk.Label(launcher_setup_frame, text="Launcher Directory:")
launcher_directory_label.pack(anchor='w', padx=(10, 0))

launcher_entry_frame = tk.Frame(launcher_setup_frame)
launcher_entry_frame.pack()

launcher_entry = tk.Entry(launcher_entry_frame, width=50)
launcher_entry.pack(side='left', padx=(10, 10))
tk.Button(launcher_entry_frame, text="Browse", command=browse_launcher).pack(side='right', padx=(10, 10))
...
export_profile_button = tk.Button(profile_listbox_frame, text="Export Profile",
                                  command=lambda: profile_management.export_profile(profile_listbox,
                                                                                    profile_entry))
export_profile_button.pack(fill='x', padx=5, pady=2)
import_profile_button = tk.Button(profile_listbox_frame, text="Import Profile",
                                  command=lambda: profile_management.import_profile(savestate_listbox,
                                                                                    profile_listbox,
                                                                                    profile_entry))
import_profile_button.pack(fill='x', padx=5, pady=2)
...
rename_savestate_button = tk.Button(savestate_listbox_frame, text="Rename Savestate",
                                    command=lambda: save_management.rename_savestate(profile_listbox,
                                                                                     savestate_listbox,
                                                                                     profile_entry,
                                                                                     notification))
rename_savestate_button.pack(fill='x', padx=5, pady=2)
...
launch_button = tk.Button(config_frame, text="Launch Bloodborne Launcher", command=lambda:launch_bloodborne_launcher())
launch_button.pack(fill='x', padx=5, pady=2)
